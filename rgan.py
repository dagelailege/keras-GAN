...
def generator_model():
    model = Sequential()
    model.add(LSTM(32, input_shape = (25, 4), return_sequences=True))
    model.add(Dense(1))
...
def draw_sin(generated_sin,epoch,batch):
    bz = generated_sin.shape[0]
    ts = generated_sin.shape[1]
    plt.figure()
    print (bz, ts)
    for i in range(bz):
        plt.subplot(8,8,i+1)
        plt.ylim(-1, 1)
        plt.axis('off')
        plt.plot(generated_sin[i,:], color = 'red')
        #exit(0)
    plt.savefig("./sin_images/{}_{}.png".format(epoch, batch))
